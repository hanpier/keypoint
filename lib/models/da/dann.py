...
class DANN():
    ...
    def build_critic(self):
        ...
        critic_body = MLP(in_features=fdim, hidden_layers=[fdim, fdim], activation="relu")
        self.critic = nn.Sequential(critic_body, nn.Linear(fdim, 1))
        print("# params: {:,}".format(count_num_param(self.critic)))
        self.critic.to(self.device)
        self.optim_c = build_optimizer(self.critic, self.config.OPTIM)
        self.sched_c = build_lr_scheduler(self.optim_c, self.config.OPTIM)
