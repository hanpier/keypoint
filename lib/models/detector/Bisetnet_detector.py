from ..backbone import BiSeNetV1
from ..head import Bisetnet_Head
...
class Bisetnet_Detector(nn.Module):
    def __init__(self, config, num_classes=16, mode='train'):
        super(Bisetnet_Detector, self).__init__()

        self.stem = CompressingNetwork()

        self.backbone = BiSeNetV1(n_classes=16)

        self.header = Bisetnet_Head()
