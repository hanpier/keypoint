...
class BoneLoss(nn.Module):
    ...
    def forward(self,
        ...
        def _Frobenius(bone_pre, bone_gt):
            return torch.pow(torch.sum(torch.pow(bone_pre - bone_gt, 2)/target.shape[2]/target.shape[3]), 0.5)

        _bone_idx = [[0, 1], [1, 2], [2, 3], [4, 5], [5, 6], [6, 7], [8, 9], [9, 10],
                     [10, 11], [12, 13], [13, 14], [14, 15], [0, 5], [5, 10], [10, 15],
                     [0, 5], [5, 10], [10, 15], [0, 15], [2, 7], [7, 8], [8, 13], [2, 13]]
        _loss = 0
        _mask = self._get_mask(target, target_weights, mask)
        for bone_id in _bone_idx:
            bone_pre = output[:, bone_id[0], :, :] - output[:, bone_id[1], :, :]
            bone_gt = target[:, bone_id[0], :, :] - target[:, bone_id[1], :, :]

            f = _Frobenius(bone_pre, bone_gt)
            _loss += f
        _loss = _loss / len(_bone_idx)
        if _mask is None:
            loss = _loss
        else:
