...
from lib.utils.reverse_grad import ReverseGrad
...
class Transfer_trainer:
    def __init__(self, config):
        ...
        # dataloader
        self.labeled_train_loader = get_train_loader(config=self.config, type='labeled', drop_last=False, trans=False)
        self.test_loader = get_test_loader(config=self.config, type='labeled', drop_last=False)
        self.unlabeled_train_loader = get_train_loader(config=self.config, type='unlabeled', drop_last=False, trans=False)
        self.len_loader = len(self.labeled_train_loader)
        ...
        self.domain_model = MLP()
        self.model = DataParallel(self.model, device_ids=[0]).to(self.device)
        self.domain_model = DataParallel(self.domain_model, device_ids=[0]).to(self.device)
        # optimizer
        self.optimizer = build_optimizer(optimizer_cfg=self.config.optimizer_cfg, model=self.model)
        self.domain_optimizer = build_optimizer(optimizer_cfg=self.config.transfer_optimizer_cfg, model=self.domain_model)
        ...
        self.lr_scheduler = LinearWarmupCosineAnnealingLR(optimizer=self.optimizer,
                                                          ...
                                                          eta_min=0)
        self.domain_lr_scheduler = LinearWarmupCosineAnnealingLR(optimizer=self.domain_optimizer,
                                                          warmup_epochs=self.config.warmup_epoch * self.len_loader,
                                                          max_epochs=self.config.joint_epoch * self.len_loader,
                                                          warmup_start_lr_factor=self.config.start_factor,
                                                          eta_min=0)
    ...
    def train_epoch(self, epoch):
        ...
        pbar = tqdm(enumerate(range(self.len_loader)), total=self.len_loader,
                    ...
                    ncols=0)
        label_dataloader = iter(self.labeled_train_loader)
        unlabel_dataloader = iter(self.unlabeled_train_loader)
        ...
        for i, _ in pbar:
            ...
            label_batch = next(label_dataloader)
            unlabel_batch = next(unlabel_dataloader)

            label_batch_image = label_batch['img'].to(self.device)
            label_batch_heatmap_gt = label_batch['heatmap'].to(self.device)
            unlabeled_batch_image = unlabel_batch['img'].to(self.device)
            num_item = label_batch_image.shape[0]

            # for dann
            domain_x = torch.ones(label_batch_image.shape[0], 1).to(self.device)
            domain_u = torch.zeros(unlabeled_batch_image.shape[0], 1).to(self.device)

            # Labeled images
            # forward through model
            label_batch_heatmap_pre, feat_x = self.model(label_batch_image)
            _, feat_u = self.model(unlabeled_batch_image)

            supervised_loss = self.supervised_criterion(label_batch_heatmap_pre,
                                                        label_batch_heatmap_gt)
            bone_loss = self.bone_criterion(label_batch_heatmap_pre,
                                            label_batch_heatmap_gt, loss_weight=self.config.bone_weight)

            # for dann
            global_step = i + 1 + (self.current_epoch + 1) * (self.len_loader + 1)
            progress = global_step / ((self.config.joint_epoch + 1) * (i + 1))
            lmda = 2 / (1 + np.exp(-10 * progress)) - 1
            feat_x = self.revgrad(feat_x, grad_scaling=lmda)
            feat_u = self.revgrad(feat_u, grad_scaling=lmda)
            output_xd = self.domain_model(feat_x)
            output_ud = self.domain_model(feat_u)
            loss_d = (self.bce(output_xd, domain_x) + self.bce(output_ud, domain_u)) * self.config.w_d

            supervised_loss_meter.update(val=supervised_loss.item(),
                                         weight=num_item)
            bone_loss_meter.update(val=bone_loss.item(),
                                   weight=num_item)
            loss_d_meter.update(val=loss_d.item(),
                                weight=num_item)

            loss_total = supervised_loss + bone_loss + loss_d
            ...
            clip_grad_norm_(self.domain_model.parameters(), self.config.clip)
        ...
        result = {'train/supervised_loss': round(supervised_loss_meter.average(), 8),
                  ...
                  'train/loss_d': round(loss_d_meter.average(), 8),
    ...
    def train(self, resume=False, pre=False):
        if resume:
            self.load_checkpoint_from_resume(os.path.join(self.checkpoint_path,self.config.resume_path))
            ...
            if pre:
                self.load_checkpoint_from_pt(os.path.join(self.checkpoint_path, self.config.resume_path))
